...
function s.spfilter(c,e,tp,check)
	...
	and (c:IsLocation(LOCATION_HAND) or (check and not c:IsCode(100214002)))
end
function s.checkfilter(c)
	return c:IsFaceup() and c:IsCode(100214003)
end
function s.sptg1(e,tp,eg,ep,ev,re,r,rp,chk)
  ...
  if check then Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND)
	else Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,0)
  end
end
