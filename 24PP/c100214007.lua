...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and aux.IsInGroup(chkc,g) end
	if chk==0 then return Duel.IsExistingTarget(aux.IsInGroup,tp,LOCATION_GRAVE,0,1,nil,g) end
	local sg
	if g:GetCount()==1 then
		...
		sg=Duel.SelectTarget(tp,aux.IsInGroup,tp,LOCATION_GRAVE,0,1,1,nil,g)
	end
	Duel.SetOperationInfo(0,HINTMSG_ATOHAND,sg,1,0,0)
end
