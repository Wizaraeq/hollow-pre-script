--蘇りし天空神
function c100292050.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(100292050,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_DRAW)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetProperty(EFFECT_FLAG_CANNOT_INACTIVATE+EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_CANNOT_NEGATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetHintTiming(0,TIMING_STANDBY_PHASE)
	e1:SetCountLimit(1,100292050)
	e1:SetTarget(c100292050.target)
	e1:SetOperation(c100292050.activate)
	c:RegisterEffect(e1)
	--Place Deck
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(100292050,1))
	...
	e2:SetCountLimit(1,100292050+100)
	e2:SetCost(aux.bfgcost)
	e2:SetTarget(c100292050.tdtg)
	e2:SetOperation(c100292050.tdop)
	c:RegisterEffect(e2)
end
function c100292050.spfilter(c,e,tp)
	return c:IsCode(10000020) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP)
end
function c100292050.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local ct1=6-Duel.GetFieldGroupCount(tp,LOCATION_HAND,0)
	local ct2=6-Duel.GetFieldGroupCount(tp,0,LOCATION_HAND)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c100292050.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp)
...
function c100292050.activate(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c100292050.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
...
function c100292050.tdfilter(c,deck_count)
...
function c100292050.tdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local deck_count=Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)
	if chk==0 then return Duel.IsExistingMatchingCard(c100292050.tdfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil,deck_count) end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,tp,LOCATION_GRAVE+LOCATION_DECK)
end
function c100292050.tdop(e,tp,eg,ep,ev,re,r,rp)
	local deck_count=Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(100292050,2))
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c100292050.tdfilter),tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil,deck_count)
	local tc=g:GetFirst()
	if tc then
		Duel.ShuffleDeck(tp)
