...
function c100417014.initial_effect(c)
	...
	e1:SetTarget(c100417014.hsptg)
	e1:SetOperation(c100417014.hspop)
	c:RegisterEffect(e1)
	-- Special Summon Xyz
...
function c100417014.hspfilter(c,e,tp)
	return c:IsSetCard(0x271) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c100417014.hsptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c100417014.hspfilter,tp,LOCATION_HAND,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND)
end
function c100417014.cfilter(c)
	return c:IsFaceup() and c:IsCode(100417013)
end
function c100417014.hspop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c100417014.hspfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp)
...
function c100417014.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
