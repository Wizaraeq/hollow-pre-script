...
function c100417015.initial_effect(c)
	...
	e1:SetCost(c100417015.drcost)
	e1:SetTarget(c100417015.drtg)
	e1:SetOperation(c100417015.drop)
	c:RegisterEffect(e1)
	-- Special Summon Xyz
...
function c100417015.drcostfilter(c)
	return c:IsSetCard(0x271) and c:IsAbleToDeckAsCost()
end
function c100417015.drcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c100417015.drcostfilter,tp,LOCATION_HAND,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectMatchingCard(tp,c100417015.drcostfilter,tp,LOCATION_HAND,0,1,1,nil)
...
function c100417015.drtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDraw(tp,1) end
	Duel.SetTargetPlayer(tp)
	Duel.SetTargetParam(1)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
	if Duel.IsExistingMatchingCard(c100417015.cfilter,tp,LOCATION_ONFIELD,0,1,nil) then
		Duel.SetOperationInfo(0,CATEGORY_RECOVER,nil,0,tp,800)
	end
end
function c100417015.drop(e,tp,eg,ep,ev,re,r,rp)
...
function c100417015.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
