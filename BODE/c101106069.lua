...
function getcheck(c,mc)
	local flag,check=0,false
	...
	if flag==1 then check=true  
	else check=false end
	return check
end
function c101106069.filter1(c)
	return c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost(POS_FACEDOWN) and not c:IsPublic()
		and Duel.IsExistingMatchingCard(c101106069.filter2,tp,LOCATION_DECK,0,1,nil,c)
end
function c101106069.filter2(c,mc)
	local check=getcheck(c,mc)
	return c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost(POS_FACEDOWN) and check
		and Duel.IsExistingMatchingCard(c101106069.filter3,tp,LOCATION_DECK,0,1,nil,c)
end
function c101106069.filter3(c,mc)
	local check=getcheck(c,mc)
	return c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and check
end
function c101106069.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c101106069.filter1,tp,LOCATION_HAND,0,1,nil) end
...
function c101106069.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1=Duel.SelectMatchingCard(tp,c101106069.filter1,tp,LOCATION_HAND,0,1,1,nil)
	Duel.ConfirmCards(1-tp,g1)
	if g1:GetCount()==0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
	local g2=Duel.SelectMatchingCard(tp,c101106069.filter2,tp,LOCATION_DECK,0,1,1,nil,g1:GetFirst())
