...
function c101106066.spfilter(c,e,tp)
	return c:IsCode(101106040) and c:IsCanBeSpecialSummoned(e,0,tp,true,false) and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0
end
function c101106066.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local loc=LOCATION_HAND+LOCATION_ONFIELD
	if c101106066.deckcon(tp) then loc=loc|LOCATION_DECK end
	if chk==0 then return Duel.IsExistingMatchingCard(c101106066.cfilter,tp,loc,0,2,nil)
		and Duel.IsExistingMatchingCard(c101106066.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,nil,2,tp,loc)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_EXTRA)
end
function c101106066.fselect(g)
...
function c101106066.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local rg=g:SelectSubGroup(tp,c101106066.fselect,false,2,2)
	if #rg==2 and Duel.Remove(rg,POS_FACEUP,REASON_EFFECT)~=0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local sg=Duel.SelectMatchingCard(tp,c101106066.spfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp)
		if #sg>0 then
