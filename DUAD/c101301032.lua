...
function s.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCodeFun(c,101301008,aux.FilterBoolFunction(Card.IsFusionSetCard,0x2cd),1,true,true)
	--position change
...
function s.negtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if re:GetHandler():IsDestructable() and re:GetHandler():IsRelateToChain(re) then
...
function s.spfilter(c,e,tp)
	return c:IsFaceupEx() and c:IsCode(101301008) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
