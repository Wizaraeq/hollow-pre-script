...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then 
		...
		return not Duel.IsPlayerAffectedByEffect(tp,59822133)
			and Duel.GetLocationCount(tp,LOCATION_MZONE)>=3 and g:CheckSubGroup(s.sprescon,3,3)
	end
end
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local sg1=g:SelectSubGroup(tp,s.sprescon,false,3,3,e,tp)
	if sg1 and #sg1==3 then
