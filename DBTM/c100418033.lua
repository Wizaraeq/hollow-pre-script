...
function c100418033.desfilter(c)
	return c:IsSummonType(SUMMON_TYPE_SPECIAL)
end
function c100418033.spfilter(c,e,tp,ec)
...
function c100418033.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local b2=Duel.IsExistingMatchingCard(c100418033.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
	if chk==0 then return b1 or b2 end
	local op=0
	if b1 and b2 then
		op=Duel.SelectOption(tp,aux.Stringid(100418033,0),aux.Stringid(100418033,1))
	elseif b1 then
		op=Duel.SelectOption(tp,aux.Stringid(100418033,0))
	else
		op=Duel.SelectOption(tp,aux.Stringid(100418033,1))+1
	end
	e:SetLabel(op)
	if op==0 then
...
function c100418033.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.GetFirstTarget()
		...
		local rg=Duel.GetDecktopGroup(1-tp,3)
			if rg:GetCount()>0 and rg:FilterCount(Card.IsAbleToRemove,nil)==3 then
		...
		local g=Duel.SelectMatchingCard(tp,c100418033.spfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,c)
