...
function c100418034.desfilter(c)
	return c:IsSummonType(SUMMON_TYPE_SPECIAL)
end
function c100418034.spfilter(c,e,tp,ec)
...
function c100418034.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsFaceup() end
	local b1=Duel.IsExistingTarget(Card.IsFaceup,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil)
	local b2=Duel.IsExistingMatchingCard(c100418034.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
	if chk==0 then return b1 or b2 end
	local op=0
	if b1 and b2 then
		op=Duel.SelectOption(tp,aux.Stringid(100418034,0),aux.Stringid(100418034,1))
	elseif b1 then
		op=Duel.SelectOption(tp,aux.Stringid(100418034,0))
	else
		op=Duel.SelectOption(tp,aux.Stringid(100418034,1))+1
	end
	e:SetLabel(op)
	if op==0 then
		e:SetCategory(0)
...
function c100418034.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if e:GetLabel()==0 then
		...
		local g=Duel.SelectMatchingCard(tp,c100418034.spfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,c)
