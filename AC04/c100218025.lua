...
function s.filter1(c,e,tp)
	return c:IsRace(RACE_SPELLCASTER) and c:IsType(TYPE_XYZ) and c:IsRank(4) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		and aux.MustMaterialCheck(c,tp,EFFECT_MUST_BE_XMATERIAL)
		and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c)
end
function s.filter2(c,e,tp,mc)
	return c:IsRank(5) and c:IsRace(RACE_SPELLCASTER) and mc:IsCanBeXyzMaterial(c)
