--金科玉条
--Script by 奥克斯
function c100297004.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetCountLimit(1,100297004+EFFECT_COUNT_CODE_OATH)
	e1:SetTarget(c100297004.target)
	e1:SetOperation(c100297004.operation)
	c:RegisterEffect(e1)
	--destroy
	...
	e2:SetOperation(c100297004.checkop)
	c:RegisterEffect(e2)
	local e3=Effect.CreateEffect(c)
	e3:SetType(EFFECT_TYPE_CONTINUOUS+EFFECT_TYPE_SINGLE)
	e3:SetCode(EVENT_LEAVE_FIELD)
	e3:SetOperation(c100297004.desop)
	e3:SetLabelObject(e2)
	c:RegisterEffect(e3)
end
function c100297004.filter(c)
	return c:IsSetCard(0x1034) and c:IsType(TYPE_MONSTER) and not c:IsForbidden()
end
function c100297004.spfilter(c,e,tp,code1,code2)
...
function c100297004.gcheck(g,e,tp)
	...
	if ft>1 then
		...
		if Duel.MoveToField(ac,tp,tp,LOCATION_SZONE,POS_FACEUP,true)
			...
			if Duel.SpecialSummonStep(tc,0,tp,tp,false,false,POS_FACEUP) then
				...
				e1:SetValue(c100297004.eqlimit)
				c:RegisterEffect(e1)
			end
...
function c100297004.eqlimit(e,c)
	return e:GetOwner()==c
end
function c100297004.checkop(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():IsDisabled() then
		e:SetLabel(1)
	else e:SetLabel(0) end
end
function c100297004.desop(e,tp,eg,ep,ev,re,r,rp)
