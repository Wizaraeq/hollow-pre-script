...
function s.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x2bd) and c:IsType(TYPE_XYZ)
end
...
function s.matfilter(sg,tp,g)
	if sg:Filter(Card.IsSetCard,nil,0x2bd):GetCount()==0 then return false end
