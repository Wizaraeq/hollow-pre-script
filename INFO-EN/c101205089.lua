...
function s.ffilter(c,fc,sub,mg,sg)
	return c:IsRace(RACE_FIEND) and (not sg or sg:FilterCount(aux.TRUE,c)==0 or sg:IsExists(Card.IsAttribute,1,c,c:GetAttribute()))
end
...
function s.rmvfiler(c)
	return c:IsRace(RACE_FIEND) and c:IsLevelAbove(6) and c:IsAbleToRemoveAsCost() and not c:IsCode(id)
end
