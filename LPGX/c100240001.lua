...
function s.cfilter(c)
	return c:IsControlerCanBeChanged() and c:GetCounter(0x1175)>0
end
function s.cctg(e,tp,eg,ep,ev,re,r,rp,chk)
	local b1=Duel.IsExistingMatchingCard(Card.IsCanAddCounter,tp,0,LOCATION_MZONE,1,nil,0x1175,1)
	...
	if op==1 then
		...
		local g=Duel.GetMatchingGroup(Card.IsCanAddCounter,tp,0,LOCATION_MZONE,nil,0x1175,1)
...
function s.ccop(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if e:GetLabel()==1 then
		...
		local g=Duel.SelectMatchingCard(tp,Card.IsCanAddCounter,tp,0,LOCATION_MZONE,1,1,nil,0x1175,1)
		local tc=g:GetFirst()
		if tc then
			Duel.HintSelection(g)
			tc:AddCounter(0x1175,1)
		end
	else
