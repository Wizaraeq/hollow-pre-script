...
function s.thfilter(c)
	return c:IsAbleToHand() and (c:IsSetCard(0x7b) or c:IsSetCard(0x2b4))
end
...
function s.spfilter(c,e,tp,mc)
	return c:IsType(TYPE_XYZ) and c:IsRank(8) and c:IsRace(RACE_DRAGON) and c:GetAttack()==3000
...
function s.matfilter(c)
	return c:IsSetCard(0x7b) and c:IsType(TYPE_MONSTER) and c:IsCanOverlay()
end
