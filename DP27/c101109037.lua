	...
	aux.rit_mat_hack_check=true
		...
		function Duel.GetRitualMaterial(tp)
			...
			for xc in aux.Next(xg) do
				g:Merge(xg)
			aux.RCheckAdditional=c101109037.rcheck
			aux.RGCheckAdditional=c101109037.rgcheck
			end
			return g
		end
		function Duel.ReleaseRitualMaterial(mat)
			...
			local tc=xmat:GetFirst()
			local te=tc:IsHasEffect(101109037,tp)
			if te then
				te:UseCountLimit(tp)
			end
...
function c101109037.rcheck(tp,g,c)
	return g:FilterCount(Card.IsLocation,nil,LOCATION_DECK)<=1
end
function c101109037.rgcheck(g)
	return g:FilterCount(Card.IsLocation,nil,LOCATION_DECK)<=1
end