--暗黒界の龍神王 グラファ
--scripted by JoyJ
function c100313051.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCodeFun(c,34230233,aux.FilterBoolFunction(Card.IsFusionAttribute,ATTRIBUTE_DARK),1,true,true)
	--change effect
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(100313051,0))
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetCode(EVENT_CHAINING)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCountLimit(1,100313051)
	e1:SetCondition(c100313051.chcon)
	e1:SetTarget(c100313051.chtg)
	e1:SetOperation(c100313051.chop)
	c:RegisterEffect(e1)
	--special summon
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(100313051,1))
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_HANDES)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e2:SetCode(EVENT_LEAVE_FIELD)
	e2:SetProperty(EFFECT_FLAG_DELAY)
	e2:SetCondition(c100313051.spcon)
	e2:SetTarget(c100313051.sptg)
	e2:SetOperation(c100313051.spop)
	c:RegisterEffect(e2)
end
...
function c100313051.chop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.ChangeChainOperation(ev,c100313051.repop)
end
function c100313051.repop(e,tp,eg,ep,ev,re,r,rp)
