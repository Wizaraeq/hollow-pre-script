...
function c100420023.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(100420023,0))
...
function c100420023.cfilter(c,e,tp)
	return not c:IsPublic() and Duel.IsExistingMatchingCard(c100420023.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp,c)
end
function c100420023.spfilter(c,e,tp,rc)
	return c:IsSetCard(0x297) and c:IsAttribute(rc:GetAttribute())
...
function c100420023.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c100420023.cfilter,tp,LOCATION_HAND,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
	local g=Duel.SelectMatchingCard(tp,c100420023.cfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp)
...
function c100420023.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,c100420023.spfilter,tp,LOCATION_DECK,0,1,1,nil,e,tp,e:GetLabelObject()):GetFirst()
