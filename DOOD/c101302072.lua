...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ct>0 and Duel.IsCanAddCounter(tp,0x70,ct,e:GetHandler()) end
	Duel.SetOperationInfo(0,CATEGORY_COUNTER,nil,ct,0,0x70)
end
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local ct=Duel.GetMatchingGroupCount(aux.TRUE,tp,0,LOCATION_ONFIELD,nil)
	if c:IsRelateToChain() and ct>0 then
		c:AddCounter(0x70,ct)
	end
end
