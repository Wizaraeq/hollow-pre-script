...
function c101108012.sclawfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0x27b)
end
function c101108012.checkzone(tp)
	local zone=0
	local lg=Duel.GetMatchingGroup(c101108012.sclawfilter,tp,LOCATION_MZONE,0,nil)
...
function c101108012.hspcon(e,c)
	...
	local zone=c101108012.checkzone(tp)
...
function c101108012.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetMatchingGroupCount(Card.IsDefensePos,tp,LOCATION_MZONE,LOCATION_MZONE,nil)>=3
		and Duel.IsPlayerCanDraw(tp,1) and Duel.SelectYesNo(tp,aux.Stringid(101108012,1)) then
		Duel.BreakEffect()
