...
function c101108024.initial_effect(c)
	...
	e3:SetCost(c101108024.costchk)
	e3:SetOperation(c101108024.costop)
	c:RegisterEffect(e3)
	--accumulate
	local e4=Effect.CreateEffect(c)
	e4:SetType(EFFECT_TYPE_FIELD)
	e4:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e4:SetCode(0x10000000+101108024)
	e4:SetRange(LOCATION_MZONE)
	e4:SetTargetRange(0,1)
	c:RegisterEffect(e4)
	--remove
	local e5=Effect.CreateEffect(c)
	e5:SetDescription(aux.Stringid(101108024,1))
	e5:SetCategory(CATEGORY_REMOVE+CATEGORY_DRAW)
	e5:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e5:SetProperty(EFFECT_FLAG_DELAY+EFFECT_FLAG_CARD_TARGET)
	e5:SetCode(EVENT_TO_GRAVE)
	e5:SetRange(LOCATION_MZONE)
	e5:SetCountLimit(1,101108024+100)
	e5:SetCondition(c101108024.rmcon)
	e5:SetTarget(c101108024.rmtg)
	e5:SetOperation(c101108024.rmop)
	c:RegisterEffect(e5)
end
...
function c101108024.cfilter(c)
	return c:IsSetCard(0x137) and c:IsLevelAbove(7) and c:IsFaceup()
end
function c101108024.costcon(e)
	return Duel.IsExistingMatchingCard(c101108024.cfilter,e:GetHandlerPlayer(),LOCATION_MZONE,0,1,nil)
end
