...
function s.initial_effect(c)
	--Change its level to the level of another "Gimmick Puppet" monster
	...
	c:RegisterEffect(e2)
	--Increase the levels of up to 2 "Gimmick Puppet" monsters by 4
...
function s.costfilter(c,lv)
	return c:IsSetCard(0x1083) and c:IsType(TYPE_MONSTER) and not c:IsLevel(lv) and c:IsAbleToGraveAsCost()
end
...
function s.lvtgfilter(c)
	return c:IsSetCard(0x1083) and c:IsFaceup() and c:IsLevelAbove(1)
end
