...
function s.thfilter(c)
	return c:IsSetCard(0x2b3) and c:IsType(TYPE_MONSTER) and not c:IsCode(id) and c:IsAbleToHand()
end
...
function s.thconfilter(c,tp)
	return c:IsControler(tp) and c:IsFaceup() and c:IsAttribute(ATTRIBUTE_LIGHT)
		and c:IsRace(RACE_SPELLCASTER) and c:IsType(TYPE_TUNER)
end
function s.thcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnPlayer()==1-tp and eg:IsExists(s.thconfilter,1,nil,tp)
end
