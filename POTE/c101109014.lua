...
function c101109014.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		...
		and Duel.IsExistingMatchingCard(c101109014.tgfilter,tp,LOCATION_HAND,0,1,c) end
...
function c101109014.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local chkf=tp
		local mg=Duel.GetMatchingGroup(c101109014.filter0,tp,LOCATION_HAND+LOCATION_MZONE+LOCATION_GRAVE,0,nil,e)
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c101109014.filter1,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,chkf)
			end
		end
		return res
	end
...
function c101109014.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c101109014.filter1,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,chkf)
	end
	if sg1:GetCount()>0 or (sg2~=nil and sg2:GetCount()>0) then
		...
		if sg1:IsContains(tc) and (sg2==nil or not sg2:IsContains(tc) or not Duel.SelectYesNo(tp,ce:GetDescription())) then
			...
			local p=tp
			for i=1,2 do
				local dg=mat1:Filter(c101109014.seqfilter,nil,p)
				if #dg>1 then
