...
function c100206004.initial_effect(c)
	...
	e0:SetValue(c100206004.splimit)
	c:RegisterEffect(e0)
	...
	c:RegisterEffect(e1)
	--Special Summon 1 banished "Sanga of the Thunder" or "Kazejin" if this card leaves the field by opponent's card
...
function c100206004.splimit(e,se,sp,st)
	return not e:GetHandler():IsLocation(LOCATION_EXTRA)
end
function c100206004.contactfil(tp)
	return Duel.GetMatchingGroup(Card.IsAbleToRemoveAsCost,tp,LOCATION_ONFIELD,0,nil)
end
function c100206004.contactop(g)
	Duel.Remove(g,POS_FACEUP,REASON_COST+REASON_MATERIAL)
end
function c100206004.thfilter(c)
	return c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToHand() and (aux.IsCodeListed(c,25955164) or aux.IsCodeListed(c,62340868) or aux.IsCodeListed(c,98434877))
end
function c100206004.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c100206004.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c100206004.thfilter,tp,LOCATION_DECK,0,1,1,nil)
	if #g>0 then
...
function c100206004.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and
		Duel.IsExistingMatchingCard(c100206004.spfilter,tp,LOCATION_REMOVED,0,1,nil,e,tp) end
