...
function c100206007.initial_effect(c)
	...
	e2:SetTarget(c100206007.limtg)
	c:RegisterEffect(e2)
	--Place 1 Sanga, Kazejin, or Suijin face-up in your S&T Zone as a Continuous Spell
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(100206007,0))
	e3:SetType(EFFECT_TYPE_IGNITION)
	e3:SetRange(LOCATION_FZONE)
	e3:SetCountLimit(1)
	e3:SetTarget(c100206007.placetg)
	e3:SetOperation(c100206007.placeop)
	c:RegisterEffect(e3)
	--Destroy 1 opponent's monster with less than 1600 ATK
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(100206007,1))
...
function c100206007.limtg(e,c)
	return c:IsStatus(STATUS_SUMMON_TURN+STATUS_FLIP_SUMMON_TURN+STATUS_SPSUMMON_TURN) and (c:GetOriginalLevel()<5 or not c:IsLevelAbove(1))
end
function c100206007.plfilter(c)
	return c:IsCode(25955164,62340868,98434877) and not c:IsForbidden() and (c:IsFaceup() or not c:IsLocation(LOCATION_REMOVED))
end
function c100206007.placetg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingMatchingCard(c100206007.plfilter,tp,LOCATION_REMOVED+LOCATION_HAND+LOCATION_DECK,0,1,nil) end
end
function c100206007.placeop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local tc=Duel.SelectMatchingCard(tp,c100206007.plfilter,tp,LOCATION_REMOVED+LOCATION_HAND+LOCATION_DECK,0,1,1,nil):GetFirst()
