...
function s.val(e,c)
	return c:IsRace(RACE_INSECT+RACE_PLANT+RACE_REPTILE) and 300 or -300
end
function s.thfilter(c)
	return c:IsSetCard(0x2ab) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x2ab) and (c:IsFaceup() or not c:IsLocation(LOCATION_REMOVED))
