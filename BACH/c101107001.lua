...
function c101107001.initial_effect(c)
	...
	e1:SetHintTiming(0,TIMING_BATTLE_START+TIMING_BATTLE_END+TIMING_DAMAGE)
...
function c101107001.spcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return Duel.GetFlagEffect(tp,101107001)>0 and (ph==PHASE_MAIN1 or (ph>=PHASE_BATTLE_START and ph<=PHASE_BATTLE) or ph==PHASE_MAIN2)
end
...
function c101107001.cfilter(c,tp)
	return c:IsPreviousControler(tp) and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP)
		and bit.band(c:GetPreviousRaceOnField(),RACE_SPELLCASTER)~=0
		and (c:IsReason(REASON_BATTLE) or c:IsReason(REASON_EFFECT) and c:GetReasonPlayer()==1-tp)
end
function c101107001.thcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c101107001.cfilter,1,nil,tp)
end
