...
function c101107003.initial_effect(c)
	...
	e2:SetTarget(c101107003.fustg)
	e2:SetOperation(c101107003.fusop)
	c:RegisterEffect(e2)
end
function c101107003.cfilter(c,tp)
	return (c:IsLocation(LOCATION_HAND) or c:IsFaceup())
	and (c:IsRace(RACE_SPELLCASTER) or (c:IsType(TYPE_SPELL+TYPE_TRAP) and aux.IsCodeListed(c,46986414))) and c:IsAbleToGraveAsCost() and Duel.GetMZoneCount(tp,c)>0
end
function c101107003.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c101107003.cfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,e:GetHandler(),tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c101107003.cfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,1,e:GetHandler(),tp)
...
function c101107003.fustg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local chkf=tp
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c101107003.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg3,mf,chkf)
			end
		end
		return res
	end
...
function c101107003.fusop(e,tp,eg,ep,ev,re,r,rp)
	local chkf=tp
	...
	if ce~=nil then
		...
		mg3=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c101107003.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg3,mf,chkf)
	end
